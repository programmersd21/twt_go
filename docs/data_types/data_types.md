...
Defines a **behavior contract** instead of storing data. Any type that implements the required methods satisfies the interface automatically.
...
* `struct` → data shape
* `interface` → behavior shape
* If it walks like a duck and quacks like a duck → it’s a duck (Go style)

### ⚡ Key Idea

* No explicit declaration needed to implement an interface
* Focuses on **behavior over type**
* Core to Go’s polymorphism system
